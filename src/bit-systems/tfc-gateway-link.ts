...
const TFCGatewayLinkQuery = defineQuery([TFCGatewayLink]);
const TFCGatewayLinkEnterQuery = enterQuery(TFCGatewayLinkQuery);
const TFCGatewayLinkExitQuery = exitQuery(TFCGatewayLinkQuery);

// 맵 Map<eid, {gatewayData}>
const gatewayMap = new Map();

export function TFCGatewayLinkSystem(world: HubsWorld) {
    TFCGatewayLinkQuery(world).forEach((eid: number) => {
            ...
            if (gatewayData.gatewayInit) {
                ...
                if (gatewayData.gatewayInOut != inOut) {
                    ...
                    if (gatewayData.gatewayInOut) {
                        ...
                        console.log("Enter Gateway Link", gatewayData.linkUrl);
                    }
                }
            }
        }
    ...
    TFCGatewayLinkEnterQuery(world).forEach((eid: number) => {
        //console.log("Enter TFC Gateway Link Component", eid);
        // 객체 보이기 여부
        const visible = TFCGatewayLink.visible[eid] == 1 ? true : false;
        // 링크 URL
        let linkUrl = APP.getString(TFCGatewayLink.linkUrl[eid])!;
    ...
    TFCGatewayLinkExitQuery(world).forEach((eid: number) => {
        //console.log("Exit TFC Gateway Link Component", eid);
